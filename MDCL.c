...
struct NodeManager* CreateList(int NodeCount, const char* ListID)
{
	...
	if (NodeCount <= 0 || NodeCount > NODE_LIMIT) // this is <= 0 and not just <0 because of how calloc works when allocating memory
	{
		fprintf(stderr, "\nInvalid NodeCount\n");
		return NULL;
	}
...
struct Node* InsertNodeAtEnd(struct NodeManager* NodeManager) // Realloc for expanding, do error checking
{
	...
	if (NodeCount > NODE_LIMIT) // Node Limit Error Checking
	{
		fprintf(stderr, "\nNode Limit Reached\n");
		return NULL;
	}
